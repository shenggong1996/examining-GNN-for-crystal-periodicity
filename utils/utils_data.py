...
def deload_data(filename):
    ...
    if 'kappa' in filename:
        df_1 = pd.read_csv('/root/kappa/descriptors_train.csv')
        df_2 = pd.read_csv('/root/kappa/descriptors_test.csv')
        descriptors = pd.concat([df_1, df_2], axis=0)
        print (descriptors)
    else:
        descriptors = pd.read_csv('/root/descriptors_mp.csv')
    features_dict = {}
    for _, row in descriptors.iterrows():
        features_dict[str(row['material_id'])] = []
        for fea_name in descriptors.columns:
            if fea_name in feature_list:
                features_dict[str(row['material_id'])].append(row[fea_name])
    prop = []; structures = []; ids = []; idxs = []; features = []
    for index, row in tqdm(df_ori.iterrows()):
        value = df_ori.at[index, 'property']
        if 'no' not in str(value):
            cif_id = str(df_ori.at[index, 'cif_id'])
            try:
                struct = ase.io.read('/root/sample/%s.cif'%(cif_id))
                structures.append(struct)
                ids.append(cif_id)
                prop.append(float(value))
                idxs.append(index)
                if cif_id in list(features_dict.keys()):
                    features.append(features_dict[cif_id])
                else:
                    print ('feature not existed for %s'%(cif_id))
                    features.append([0 for _ in range(len(feature_list))])
            except:
                continue
    
    df = pd.DataFrame({})
    df['structure'] = structures
    df['prop'] = np.array(prop)
    df['ids'] = ids
    df['idx'] = idxs
    ...
    return df, species, np.mean(prop), np.std(prop)

def load_data(filename):
    # load data from a csv file and derive formula and species columns from structure
    df_ori = pd.read_csv(filename)
    prop = []; structures = []; ids = []; idxs = []
    for index, row in tqdm(df_ori.iterrows()):
        value = df_ori.at[index, 'property']
#        value = np.log(value+1.5)
        if 'no' not in str(value):
            cif_id = df_ori.at[index, 'cif_id']
            try:
                struct = ase.io.read('/root/sample/%s.cif'%(cif_id))
                structures.append(struct)
                ids.append(cif_id)
                if 'crystal_system_int' in filename:
                    prop.append(int(value)-1)
                else:
                    prop.append(float(value))
                idxs.append(index)
            except:
