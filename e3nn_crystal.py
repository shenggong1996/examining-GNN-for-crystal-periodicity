...
# load data
df, species, mean, std = load_data('/root/cv_training.csv') # address to training set
df_test, species_test, _, _ = load_data('/root/cv_test.csv') # address to test set
run_name = 'model_cv_' + time.strftime("%y%m%d", time.localtime())
print(run_name)
...
idx_train, idx_valid, idx_test = train_valid_test_split(df, species, valid_size=.2, test_size=.01, seed=1234, plot=False)
...
class PeriodicNetwork(Network):
    ...
    def forward(self, data: Union[tg.data.Data, Dict[str, torch.Tensor]]) -> torch.Tensor:
        ...
        if self.pool == True:
            ...
            transformed_index = torch.tensor([mapping[val.item()] for val in index], device=device)
            output = torch_scatter.scatter_mean(output.view(-1), transformed_index, dim=0)  # take mean over atoms per example
